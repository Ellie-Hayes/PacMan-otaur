...
Pacman::Pacman(int argc, char* argv[]) : Game(argc, argv), _cPacmanSpeed(0.1f), _cPlayerFrameTime(100), _cMunchieFrameTime(500), _cCherryFrameTime(700), _cMinotaurFrameTime(100), _cGearFrameTime(2000)
{
	_player = new Player();
	...
	else
	{
		...
		if (!_paused)
		{
			...
			for (int i = 0; i < _wavespawner->Minotaurs.size(); i++)
			{
				...
				if (CheckHealth(_wavespawner->Minotaurs[i]->health))
				{
			...
			for (int i = 0; i < projectilesPlayer.size(); i++)
			{
				MoveProjectiles(projectilesPlayer[i]);
				if (deleteProjectile(projectilesPlayer[i]))
				{
					projectilesPlayer.erase(projectilesPlayer.begin() + i);
					break;
				}
...
void Pacman::Input(int elapsedTime, Input::KeyboardState* state, Input::MouseState* mouseState)
{
	...
	//Mouse
	if (mouseState ->LeftButton == Input::ButtonState::PRESSED && !_mouseDown)
	{
		_mouseDown = true; 
...
void Pacman::SpawnWave()
{
	if (_wavespawner->state == _wavespawner->Spawning)
	{
		...
		{
			if (_wavespawner->nextWave >= 20) { _wavespawner->nextWave = 19; }
...
{
	if (position->X + sourceRect->Width + 64 > Graphics::GetViewportWidth()) { position->X = Graphics::GetViewportWidth() - sourceRect->Width - 64; }
...
bool Pacman::deleteProjectile(Projectile* projectile)
{
